...
`cloud-itonami-isco-7214` accepts contributions to the OSS actor, policy tests,
...
- Keep production writes behind SteelCoordGovernor.
- Never add an op to the allowlist that could finalize a
  structural-steel-erection-execution decision, authorize a crane
  lift, or override a site-safety officer's judgment — such
  proposals must remain a hard, permanent block or an always-escalate
  op, never auto-commit-eligible.
