(ns steelcoord.governor
  "SteelCoordGovernor — the independent safety/traceability layer
  ...
  a structural-steel job site. The governor never dispatches hardware
  itself, never performs structural-steel erection work, and never
  allows a proposal to finalize a structural-steel-erection-execution
  decision, authorize a crane lift, or override a site-safety
  officer's judgment — this actor coordinates JOB-SITE SCHEDULING/
  LOGISTICS ONLY. Modeled on cloud-itonami-isco-7121's
  roofcoord.governor.
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    ...
    2. no-actuation            — proposal :effect must be :propose
                                 ...
                                 structural-steel erection work; it
    ...
    3. closed op-allowlist     — :op must be one of the four
                                 ...
                                 directly finalizes structural-steel-
                                 erection execution, authorizes a
                                 crane lift, or overrides site-safety
    ...
    6. scope-exclusion         — a proposal that attempts to finalize
                                 a structural-steel-erection-execution
                                 decision, to authorize a crane lift,
                                 or to override a site-safety
                                 officer's judgment, is a hard,
                                 PERMANENT block — never overridable
                                 by human approval, regardless of
                                 confidence or stake. Detected as
                                 finalization/execution ACTION PHRASES
                                 (e.g. 'proceed with the structural-
                                 steel erection', 'authorize the crane
                                 lift', 'override the site safety
                                 officer's judgment') in free-text
                                 proposal fields, never as bare domain
                                 nouns ('steel', 'crane', 'beam') —
                                 bare-noun matching would false-trip
                                 on the default mock advisor's own
                                 routine rationale text, since this
                                 actor's entire domain is structural-
                                 steel erection job sites at height
                                 with crane-load handling. See
                                 `steelcoord.governor-test`
                                 `default-mock-advisor-proposals-never-self-trip-scope-exclusion`.

  ESCALATION invariants (:escalate? true, ALWAYS human sign-off,
  regardless of confidence):
    7. :op :flag-safety-concern always escalates (a surfaced
                                 fall-hazard/crane-condition/load-
                                 hazard concern always requires human
                                 review — the governor never resolves
                                 a safety concern itself).
...
(def supply-order-cost-threshold 15000)
...
;; PHRASES, never as bare nouns ("steel", "crane", "beam") — this
;; actor's entire domain is structural-steel erection job sites at
;; height with crane-load handling, so bare-noun matching would
;; false-trip on the default mock advisor's own routine rationale
;; text (e.g. "proposed :coordinate-supply-order for site S-1" naming
;; structural-steel materials, or a crane-schedule proposal naming
;; "crane"). See governor-test's dedicated self-trip guard.
(def ^:private scope-exclusion-phrases
  ["proceed with the structural-steel erection"
   "proceed with the structural steel erection work"
   "finalize the structural-steel-erection-execution decision"
   "finalize the structural steel erection execution decision"
   "execute the structural-steel erection directly"
   "execute the structural steel erection directly"
   "perform the structural steel erection directly"
   "dispatch the crew to perform structural steel erection"
   "authorize the crane lift"
   "authorize the crane-lift"
   "override the site safety officer's judgment"
   "override the site-safety officer"
   "bypass the site-safety officer"
   "bypass crane-lift authorization"])
...
(defn scope-exclusion-violation?
  ...
  structural-steel-erection execution, authorize a crane lift, or
  override site-safety authority. Phrased as multi-word action
  phrases (never bare nouns) so this never false-trips on legitimate
  structural-steel-domain vocabulary."
  [proposal]
...
(defn- hard-violations [{:keys [request proposal]} site-record w]
  (let [{:keys [op site-id worker-id]} proposal]
    (cond-> []
      ...
      (conj {:rule :unknown-op :detail "closed op-allowlist 外の op（施工実行・クレーン吊荷承認・安全判断の上書きにあたる op は許可されていない）"})
      ...
      (conj {:rule :scope-exclusion-violation
             :detail "構造鉄骨建方の施工判断の確定・クレーン吊荷の承認・site-safety officer の判断の上書きにあたる提案は恒久的に禁止（human 承認でも上書き不可）"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `steelcoord.store/Store`. Pure — never mutates
