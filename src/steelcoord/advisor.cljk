(ns steelcoord.advisor
  "Steel Erection Coordination Advisor — the advisor named in this
  repository's README, proposing a job-site scheduling/logistics
  coordination operation (log a work record, schedule a crew/crane
  operation, flag a safety concern, coordinate a supply order) from a
  site roster, crew roster and job-site/crane schedule. Swappable
  mock/llm; the advisor ONLY proposes — `steelcoord.governor` checks
  site/worker registration, the closed op-allowlist and scope-
  ...
  performs structural-steel erection work and never proposes to
  finalize a structural-steel-erection-execution decision, authorize
  a crane lift, or override a site-safety officer's judgment. Modeled
  on cloud-itonami-isco-7121's roofcoord.advisor.
...
(def ^:private system-prompt
  "You are a structural-steel-erection crew scheduling/logistics
   ...
   a structural-steel-erection-execution decision, never propose to
   authorize a crane lift, never propose to override a site-safety
   officer's judgment, and never propose an op outside the closed
   allowlist above. The governor checks site/worker registration and
   scope independently. Safety-concern flags and above-threshold
