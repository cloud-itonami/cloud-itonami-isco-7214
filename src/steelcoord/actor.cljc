(ns steelcoord.actor
  "SteelCoordActor — the ISCO-08 7214 structural metal preparers and
  erectors crew scheduling/logistics coordination actor as a
  ...
  resume after human sign-off. This actor coordinates JOB-SITE
  SCHEDULING/LOGISTICS ONLY — it never performs structural-steel
  erection work itself and never commits a proposal the
  SteelCoordGovernor refuses. Modeled on cloud-itonami-isco-7121's
  roofcoord.actor.
  ...
  The unconditional invariant: the Steel Erection Coordination
  Advisor can never directly commit a coordination record the
  SteelCoordGovernor refuses — every commit-record! call is gated
  behind `:decide`, and the governor's scope-exclusion rule makes any
  proposal to finalize structural-steel-erection execution, authorize
  a crane lift, or override a site-safety officer's judgment an
...
(defn build-graph
  "Build a compiled SteelCoordActor graph. `store` implements
  `steelcoord.store/Store`. `advisor` implements
  `steelcoord.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn approve!
  ...
  can never be used to approve a structural-steel-erection-execution
  finalization, a crane-lift authorization, or a site-safety-officer-
  judgment override."
  [graph thread-id]
