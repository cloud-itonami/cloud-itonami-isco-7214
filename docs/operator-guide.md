...
- safety-critical escalation path (fall-hazard, crane-condition and load-hazard concerns always reach a human)
...
directly finalize a structural-steel-erection-execution decision,
authorize a crane lift, or override a site-safety officer's
judgment — the governor's closed op-allowlist and scope-exclusion
rule are the last line of defense, not the only one; operator-side
integrations must not create a path around them.
...
structural-steel erection, authorize a crane lift, or override
