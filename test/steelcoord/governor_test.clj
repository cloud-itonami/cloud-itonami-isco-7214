(ns steelcoord.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [steelcoord.store :as store]
            [steelcoord.advisor :as advisor]
            [steelcoord.governor :as governor]))
...
(defn- supply-op [cost]
  {:op :coordinate-supply-order :effect :propose :site-id "S-1"
   :materials "structural steel W-beams" :cost cost :confidence 0.9 :stake :low
...
(deftest hard-on-unknown-op
  (testing "closed op-allowlist enforced — no op finalizes structural-steel-erection execution, authorizes a crane lift, or overrides safety authority"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (log-op) :op :finalize-structural-steel-erection-execution) st)]
      (is (:hard? v))
...
(deftest hard-and-permanent-on-scope-exclusion-finalize-structural-steel-erection
  (testing "a proposal to directly finalize a structural-steel-erection-execution decision is a hard, permanent block"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (log-op)
                                          :confidence 0.99
                                          :rationale "proceed with the structural-steel erection now, skip further review")
                            st)]
      (is (:hard? v))
...
(deftest hard-and-permanent-on-scope-exclusion-authorize-crane-lift
  (testing "a proposal to authorize a crane lift is a hard, permanent block"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (schedule-op)
                                          :confidence 0.99
                                          :rationale "authorize the crane lift for the north bay column now")
                            st)]
      (is (:hard? v))
      (is (not (:escalate? v)))
      (is (some #(= :scope-exclusion-violation (:rule %)) (:violations v))))))
...
(deftest scope-exclusion-not-triggered-by-bare-domain-nouns
  (testing "bare nouns like 'steel'/'crane'/'beam' are ordinary domain vocabulary, not finalization/override actions"
    (let [proposal {:rationale "proposed schedule-crew-operation for structural steel job site S-1 at height near the crane"
                     :description "crew fully trained on fall-protection equipment and crane signal procedures for this steel frame"}]
...
(deftest default-mock-advisor-proposals-never-self-trip-scope-exclusion
  (testing "the mock advisor's own default rationale text, across every allowlisted op, never trips the scope-exclusion guard"
    (let [st (fresh-store)
          ...
          requests [{:site-id "S-1" :op :log-work-record :worker-id "W-1" :task "bolt-up north bay beams"}
                    {:site-id "S-1" :op :schedule-crew-operation :worker-id "W-1" :task "stage crane for column set"}
                    {:site-id "S-1" :op :flag-safety-concern :worker-id "W-1"
                     :concern-type :fall-hazard :severity :high
                     :description "loose bolt in a beam connection near an open edge at height, unsecured load swinging near the crane path"}
                    {:site-id "S-1" :op :coordinate-supply-order :materials "structural steel W-beams and connection bolts"
                     :cost 4500}]]
...
(deftest always-escalates-flag-safety-concern-even-at-high-confidence
  (testing "a surfaced fall-hazard/crane-condition/load-hazard concern always requires human review"
    (let [st (fresh-store)
